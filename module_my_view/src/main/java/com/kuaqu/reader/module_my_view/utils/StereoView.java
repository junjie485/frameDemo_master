...
public class StereoView extends ViewGroup {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (isSliding) {
                    ...
                    mDownY = y;
                    if (mScroller.isFinished()) {//上一次滑动结束
                        //因为要循环滚动
    ...
    private void toNormalAction() {
        ...
        addCount = 0;
        startY = getScrollY();
        Log.e("scroller","回弹吗"+mHeight * mStartScreen+":::"+getScrollY());
    ...
    private void toNextAction(float yVelocity) {
        ...
        System.out.println("多后一页startY " + startY + " yVelocity " + yVelocity + " delta " + delta + "  getScrollY() " + getScrollY() + " addCount " + addCount);
        duration = (Math.abs(delta)) * 3;
        mScroller.startScroll(0, startY, 0, delta, duration);
        addCount--;
    }
    ...
    private void recycleMove(int delta) {
        ...
        scrollBy(0, delta);
        Log.e("yuyutr","未进入"+getScrollY()+"==="+mHeight);
    ...
    public StereoView setItem(int itemId) {
       ...
       System.out.println("之前curScreen " + mCurScreen);
        if (!mScroller.isFinished()) {
            mScroller.abortAnimation();
            System.out.println("强制完成");
        }
        ...
        System.out.println("之后curScreen " + mCurScreen + " getScrollY " + getScrollY());
        return this;
    }
