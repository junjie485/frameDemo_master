...
public class VideoPartActivity extends AppCompatActivity {
    ...
    private void exactorMedia() {
        ...
        try {
            File videoFile = new File(SDCARD_PATH, "output_video");
            if (!videoFile.exists()) {
            ...
            mediaExtractor.setDataSource(SDCARD_PATH + "/input.mp4");//分离音视频路径
            int trackCount = mediaExtractor.getTrackCount();//获取源的信道数，音频信道、视频信道
            int audioTrackIndex = -1;
            int videoTrackIndex = -1;
            for (int i = 0; i < trackCount; i++) {
                MediaFormat trackFormat = mediaExtractor.getTrackFormat(i);//获取信道格式
            ...
            mediaExtractor.selectTrack(videoTrackIndex);//选择信道
            while (true) {
                int readSampleCount = mediaExtractor.readSampleData(byteBuffer, 0);//读取信道数据
    ...
    private void combineVideo() {
        try {
            ...
            int writeVideoTrackIndex = mediaMuxer.addTrack(videoFormat);//添加通道
            int writeAudioTrackIndex = mediaMuxer.addTrack(audioFormat);
            mediaMuxer.start();//开始合成文件
            ByteBuffer byteBuffer = ByteBuffer.allocate(500 * 1024);
            long sampleTime = 0;
            {
